...
public interface FlightPackageRepo extends JpaRepository<FlightPackage, UUID> {
    ...
    @Query(value = "SELECT al.name AS airline, cl.name AS class_type, st.name AS seat_type, BIN_TO_UUID(fp.id) AS id," +
            ...
            "WHERE (:airline IS NULL OR al.name=:airline) " +
            "AND (:destination IS NULL OR ap2.name=:destination) " +
            "AND (:departure IS NULL OR ap1.name=:departure) ", nativeQuery = true)
    Page<List<FlightPackageProjection> >fetchAllFlightPackages(
